...
const FarmFinance = () => {
  ...
  const calculateLoan = () => {
    ...
    setToast({
      message: `Monthly Payment: KES ${monthlyPayment.toLocaleString(undefined, {maximumFractionDigits: 0})} | Total Interest: KES ${totalInterest.toLocaleString(undefined, {maximumFractionDigits: 0})}`,
      type: 'success'
    });
  };

  const applyForLoan = (loanType) => {
    setToast({ message: `Application for ${loanType} submitted! We'll contact you within 24 hours.`, type: 'success' });
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loanProducts.map((loan, index) => (
                <div key={index} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <button
                    onClick={() => applyForLoan(loan.name)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {insuranceProducts.map((insurance, index) => (
                <div key={index} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <button
                    onClick={() => setToast({ message: `${insurance.name} quote requested! Agent will call you.`, type: 'success' })}
