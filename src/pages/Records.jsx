...
import {
  Loader, Trash2, BookOpen, Clock, ClipboardList, PiggyBank, X, Plus, AlertTriangle
} from 'lucide-react';
import { 
  doc, deleteDoc, onSnapshot, collection, query, addDoc, serverTimestamp, orderBy 
} from 'firebase/firestore';
import { useFirebase } from '../contexts/FirebaseContext'; 
// NOTE: Navbar was in the HEAD version but is NOT used in the resolved structure 
// that incorporates the new UI/logic. I've left it out as it conflicts with the new header.
...
const DiagnosisHistory = ({ db, userId }) => {
  ...
  React.useEffect(() => {
    ...
    // Correctly build the nested collection query: 'users/{userId}/diagnosis_records'
  ...
  const handleDelete = async (recordId) => {
    ...
    try {
      // Correctly build the nested document reference for deletion
...
const FinancialManagement = ({ db, userId }) => {
  ...
  React.useEffect(() => {
    ...
    // Correctly build the nested collection query: 'users/{userId}/management_records'
    const recordsQuery = query(
  ...
  const handleAdd = async (e) => {
    ...
    try {
      // Correctly build the nested collection reference for adding a document
